...
def initialize(context):
    ...
    g.stock_pool = ['600012.XSHG', '600007.XSHG', '600548.XSHG', '600519.XSHG', '002294.XSHE',
                    '000975.XSHE', '600566.XSHG', '000895.XSHE', '600763.XSHG', '600377.XSHG', '000858.XSHE',
                    '002007.XSHE',
                    '002304.XSHE',
                    '601888.XSHG',
                    '601107.XSHG',
...
def before_market_open(context):
    ...
    for code in g.stock_pool:
        df = attribute_history(code, look_ahead_n, "1d", [
                               "close"], skip_paused=True)  # 该函数返回结果不包括当天数据
        if len(df) != look_ahead_n:
            continue
        close = df["close"]
        ma_short = pd.Series.rolling(close, window=MA_WIN_1).mean()  # 短时均线
        ma_long = pd.Series.rolling(close, window=MA_WIN_2).mean()   # 长时均线
        uc_flags = (ma_short.shift(1) <= ma_long.shift(1)) & (
            ma_short > ma_long)  # 上穿标志
        dc_flags = (ma_short.shift(1) >= ma_long.shift(1)) & (
            ma_short < ma_long)  # 下穿标志
        if uc_flags.iloc[-1]:

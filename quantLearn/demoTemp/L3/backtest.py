...
def backtest(begin_date, end_date):
    ...
    for _date in all_dates:
        ...
        if last_date is not None and len(before_sell_holding_codes) > 0:
            ...
            for current_daily in current_daily_cursor:
                ...
                if code in code_last_aufactor_dict:
                    ...
                    # 相关公式是：
                    # 市值不变：last_close * last_volume = current_pre_close * current_volume
                    # 价格的关系：last_close * last_aufactor = current_pre_close * current_aufactor
                    # 转换之后得到下面的公式：
        ...
        print('待卖股票集合：', to_be_sold_codes, flush=True)
        # 如果有待卖股票，则继续处理
        ...
        """
        print('待买股票集合：', to_be_bought_codes, flush=True)
        # 如果待买股票集合不为空，则执行买入操作
